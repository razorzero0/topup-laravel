                ...
                <div class="flex flex-wrap justify-center gap-3 text-md " role="group">
                    @foreach ($data['items'] as $diamond)
                        @if (!Str::contains($diamond['product_name'], 'Username'))
                            <a href="#box-payment" type="button" data-price={{ $diamond['price'] }}
                                data-name="{{ $diamond['product_name'] }}" data-code={{ $diamond['buyer_sku_code'] }}
                                class="h-20 sm:h-24 w-full sm:w-40 px-2 py-2 text-center flex  items-center justify-center font-medium border rounded-md btn  bg-slate-700 text-slate-400 border-slate-600 rounded-s-lg hover:bg-orange-400 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-orange-400 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700">
