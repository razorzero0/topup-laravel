<li class="text-sm leading-6 " id="box-information">
        ...
        <div class="relative p-6 space-y-6 leading-none rounded-lg bg-slate-800 ring-1 ring-gray-900/5">
            <div class="flex items-center space-x-4">
                <img src="{{ asset($product->image ?? '') }}"
                    class="w-16 h-16 bg-center bg-cover border rounded-md -skew-x-6" alt="logo">
                <div>
                    <h3 class="text-lg font-semibold text-white">{{ $product['name'] ?? '' }}</h3>
                    <p class="text-gray-500 text-md">{{ $product->company ?? '' }}</p>
                </div>
            </div>
