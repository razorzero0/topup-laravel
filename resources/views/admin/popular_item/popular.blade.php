        ...
        <div class="p-8 bg-white border-2 rounded-lg ">
            <div class="bg-white rounded ">
                ...
                <div class="text-center ">
                    <h4 class="text-2xl font-bold">Daftar Popular Item</h4>
                </div>
                <div class="flex justify-end mt-5">
                    ...
                    <button type="button" data-modal-target="add-category" data-modal-toggle="add-category"
                        ...
                        Popular Item</button>
                    @include('admin.popular_item.add-modal')
                </div>
                ...
                <table id="default-table">
                    ...
                    <tbody>
                        @php
                            $no = 1;
                        @endphp
                        @foreach ($data as $popular)
                            <tr>
                                <td class="font-medium text-gray-900 whitespace-nowrap">{{ $no++ }}</td>
                                <td>{{ $popular->product['name'] }}</td>
                                <td>{{ $popular->product->category['name'] }}</td>

                                <td class="">

                                    <form class="inline" method="post"
                                        action="{{ route('popular-item.destroy', $popular['id']) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
