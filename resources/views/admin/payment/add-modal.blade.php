...
<div id="add-product" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-md max-h-full p-4">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="p-3">
                <form class="space-y-4" action={{ route('payment-channel.store') }} method="post">

                    @csrf

                    <label for="image" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pilih
                        Grup Pembayaran</label>
                    <select id="image"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        name="group">
                        @foreach ($payment_unique as $pay)
                            <option value="{{ $pay['group'] }}">
                                {{ $pay['group'] }}</option>
                        @endforeach

                    </select>
                    <div>
                        <label for="" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Fee
                        </label>
                        <input type="number" name="fee" value=""
